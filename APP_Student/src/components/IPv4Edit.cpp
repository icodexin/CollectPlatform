...
IPv4Edit::IPv4Edit(QWidget* parent)
    : BaseWidget(parent), ui_lineEdit0(nullptr), ui_lineEdit1(nullptr), ui_lineEdit2(nullptr),
      ui_lineEdit3(nullptr) {
    initUI();
    initConnection();
}
...
void IPv4Edit::onTextChanged(const QString& text) {
...
bool IPv4Edit::eventFilter(QObject* watched, QEvent* event) {
    ...
    const int textLen = edit->text().length();
    const int selectLen = edit->selectedText().length();
    ...
    if (event->type() == QEvent::KeyPress) {
        ...
        if (key == Qt::Key_Backspace && index > 0 && curPos == 0 && selectLen == 0) {
            ...
            if (focusPreviousChild()) {
                // 删除上一个输入框的最后一个字符
                QLineEdit* prevEdit = qobject_cast<QLineEdit*>(focusWidget());
                if (prevEdit) {
        ...
        if (key == Qt::Key_Delete && index < 3 && curPos == textLen && selectLen == 0) {
            ...
            if (focusNextChild()) {
                // 删除下一个输入框的第一个字符
                QLineEdit* nextEdit = qobject_cast<QLineEdit*>(focusWidget());
                if (nextEdit) {
    ...
    // 输入法事件
        ...
        // 输入法输入中文时, 按下'。'键, 且不是最后一个输入框, 且光标在最后
...
void IPv4Edit::initUI() {
    ...
    auto createLineEdit = [this, minWidth, validator](const int index) {
...
void IPv4Edit::initConnection() {
    connect(ui_lineEdit0, &QLineEdit::textChanged, this, &IPv4Edit::onTextChanged);
    connect(ui_lineEdit1, &QLineEdit::textChanged, this, &IPv4Edit::onTextChanged);
    connect(ui_lineEdit2, &QLineEdit::textChanged, this, &IPv4Edit::onTextChanged);
    connect(ui_lineEdit3, &QLineEdit::textChanged, this, &IPv4Edit::onTextChanged);
}

void IPv4Edit::closeConnection() {
    disconnect(ui_lineEdit0, &QLineEdit::textChanged, this, &IPv4Edit::onTextChanged);
    disconnect(ui_lineEdit1, &QLineEdit::textChanged, this, &IPv4Edit::onTextChanged);
    disconnect(ui_lineEdit2, &QLineEdit::textChanged, this, &IPv4Edit::onTextChanged);
    disconnect(ui_lineEdit3, &QLineEdit::textChanged, this, &IPv4Edit::onTextChanged);
}
